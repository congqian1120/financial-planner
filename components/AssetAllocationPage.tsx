...
const HISTORICAL_RETURNS_DATA = [
  { period: '1 yr', max: 136.07, min: -60.78, maxYear: '1932-1933', minYear: '1931-1932' },
  { period: '5 yrs', max: 31.91, min: -13.78, maxYear: '1928-1933', minYear: '1926-1931' },
  { period: '10 yrs', max: 19.19, min: -2.69, maxYear: '1949-1959', minYear: '1929-1939' },
  { period: '15 yrs', max: 17.85, min: 0.91, maxYear: '1984-1999', minYear: '1929-1944' },
  { period: '20 yrs', max: 16.49, min: 2.66, maxYear: '1980-2000', minYear: '1929-1949' },
  { period: '25 yrs', max: 15.79, min: 5.7, maxYear: '1975-2000', minYear: '1929-1954' },
];
...
const ANALYSIS_TABS = [
  ...
  "Target's historical returns", 
  "Target's historical return comparison"
];
...
const AssetAllocationPage: React.FC<AssetAllocationPageProps> = ({ data, updateData, onBack, onNavigate }) => {
  ...
  const [subTab, setSubTab] = useState(ANALYSIS_TABS[0]);
  ...
  return (
    <div className="bg-white min-h-screen animate-in fade-in duration-300 pb-24">
      ...
      <div className="p-4 md:p-12 max-w-7xl mx-auto">
        ...
        <div className="border-b border-slate-300 grid grid-cols-2 lg:flex lg:flex-wrap lg:gap-12 mb-12">
            <button 
                onClick={() => setActiveTab(0)}
                className={`pb-3 text-xs md:text-sm font-bold border-b-4 transition-colors flex items-center justify-center text-center lg:text-left ${activeTab === 0 ? 'border-[#4d7c0f] text-slate-900' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
            >
                <span className="max-w-[150px] lg:max-w-none">Suggested target asset mix to consider</span>
            </button>
            <button 
                onClick={() => setActiveTab(1)}
                className={`pb-3 text-xs md:text-sm font-bold border-b-4 transition-colors flex items-center justify-center text-center lg:text-left ${activeTab === 1 ? 'border-[#4d7c0f] text-slate-900' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
            >
                <span className="max-w-[150px] lg:max-w-none">Explore other target asset mixes</span>
            </button>
        </div>
        ...
        {activeTab === 0 ? (
            ...
            <div className="animate-in fade-in duration-500">
                <div className="mb-12">
                    <h2 className="text-xl font-bold text-slate-800 mb-2 text-center lg:text-left">Select a target asset mix</h2>
                    <p className="text-sm text-slate-600 mb-4 max-w-5xl leading-relaxed text-center lg:text-left">
                    ...
                    <p className="text-xs text-slate-500 italic mb-10 text-center lg:text-left">
                    ...
                    {/* Fixed overlapping labels on mobile by allowing horizontal scroll with min-width */}
                    <div className="overflow-x-auto no-scrollbar -mx-4 px-4 mb-16">
                        <div className="flex justify-between items-start min-w-[750px] lg:min-w-0 max-w-6xl w-full">
        ...
        <div className="mt-24 border-t border-slate-200 pt-16">
            {/* Analysis Tabs - Responsive implementation */}
            <div className="lg:hidden mb-8">
                <label className="block text-base font-bold text-slate-800 mb-2">Select charts and tables</label>
            ...
            <div className="hidden lg:flex border-b border-slate-300 flex-wrap gap-10 mb-8 overflow-x-auto no-scrollbar">
                {ANALYSIS_TABS.map((tab) => (
                    <button 
                        key={tab}
                        onClick={() => setSubTab(tab)}
                        className={`pb-3 text-sm font-bold border-b-4 transition-colors whitespace-nowrap ${subTab === tab ? 'border-[#4d7c0f] text-slate-900' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                    >
                        {tab}
                    </button>
                ))}
            ...
            {subTab === "Target's historical returns" && (
                <div className="animate-in fade-in duration-500 flex flex-col items-center">
                    ...
                    {historicalViewMode === 'Chart' ? (
                        <div className="w-full max-w-5xl h-[350px] md:h-[480px] mt-8 flex flex-row px-2">
                            <div className="flex-1 flex flex-col relative">
                              <div className="flex-1 relative">
                                  {/* Y Axis Vertical Legend - Hidden on very small mobile to save space */}
                                  <div className="absolute -left-12 md:-left-16 top-1/2 -translate-y-1/2 -rotate-90 text-[10px] md:text-[12px] font-bold text-slate-500 uppercase tracking-widest whitespace-nowrap hidden xs:block">Annualized returns (%)</div>
                                      ...
                                      >
                                          ...
                                          <Bar dataKey="posRange" radius={[2, 2, 0, 0]} barSize={40}>
                                              ...
                                              <LabelList 
                                                  dataKey="posRange" 
                                                  position="top" 
                                                  formatter={(val: [number, number]) => `+${Math.round(val[1])}%`} 
                                          ...
                                          <Bar dataKey="negRange" radius={0} barSize={40}>
                                              ...
                                              <LabelList 
                                                  dataKey="negRange" 
                                                  position="bottom" 
                                                  formatter={(val: [number, number]) => `${Math.round(val[0])}%`} 
                                                  style={{ fontSize: '10px', fontWeight: 'bold', fill: '#1e293b' }} 
                                              />
                                          </Bar>
                                      </BarChart>
                              ...
                              <div className="text-center text-[10px] md:text-[12px] text-slate-700 font-bold uppercase tracking-tight mt-4 ml-[40px] md:ml-[100px]">Amount of time assets held</div>
                            </div>
                        </div>
                    ) : (
            ...
            {subTab === "Target's historical return comparison" && (
                <div className="animate-in fade-in duration-500">
                    ...
                    <div className="w-full max-w-6xl h-[400px] md:h-[520px] flex flex-row mt-12 overflow-x-auto no-scrollbar">
                        <div className="min-w-[800px] lg:min-w-0 flex-1 flex flex-col relative px-2">
                            <div className="flex-1 relative">
                                 {/* Y Axis Vertical Legend */}
                                 <div className="absolute -left-12 md:-left-16 top-1/2 -translate-y-1/2 -rotate-90 text-[10px] md:text-[12px] font-bold text-slate-500 uppercase tracking-widest whitespace-nowrap hidden xs:block">Annualized return (%)</div>
                                 ...
                                 <ResponsiveContainer width="100%" height="100%">
                                      ...
                                      >
                                          ...
                                          <XAxis 
                                              ...
                                              axisLine={{ stroke: '#e2e8f0' }} 
                                              tickLine={false} 
                                              tick={{ fill: '#64748b', fontSize: 10, fontWeight: 700, angle: -45, textAnchor: 'end' }} 
                                              padding={{ left: 10, right: 10 }}
                                              interval={0}
                                          />
                                          ...
                                          {/* Positive Part (Green) */}
                                          <Bar dataKey="posRange" radius={[2, 2, 0, 0]} barSize={35}>
                                              ...
                                              <LabelList 
                                                  ...
                                                  position="top" 
                                                  formatter={(val: [number, number]) => `+${Math.round(val[1])}%`} 
                                                  style={{ fontSize: '9px', fontWeight: 'bold', fill: '#1e293b' }} 
                                              />
                                          </Bar>

                                          {/* Negative Part (Red) */}
                                          <Bar dataKey="negRange" radius={0} barSize={35}>
                                              {transformedComparisonData.map((entry, index) => (
                                                  <BarCell key={`neg-cell-${index}`} fill="#7f1d1d" />
                                              ))}
                                              <LabelList 
                            ...
                            <div className="text-center text-[10px] md:text-[12px] text-slate-700 font-bold uppercase tracking-tight mt-[-60px] ml-[45px] md:ml-[100px]">Asset mix</div>
                        </div>
                    </div>
                </div>
            )}
